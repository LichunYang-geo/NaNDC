...
def calc_ray_paras(evlo: float, evla: float, evdp: float, stinfo: DataFrame, vmodel: str) -> DataFrame:
    """
    ...
    for i in range(0,len(stinfo)):
        ...
        ray_para[i, 1] = tko
        ray_para[i, 2] = aoi
        ray_para[i, 3] = dist * 1000 #m
    return  DataFrame(data=ray_para, columns=['azi','tko','aoi','dist','alpha','beta','rho'])

def calc_synthetic(mts: np.ndarray, supplement: np.ndarray) -> np.ndarray:
    '''
