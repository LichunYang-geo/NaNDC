...
class DataIN():
    ...
    def __init__(self, data, evinfo, vmodel):
        ...
        self.zero_pol_index = np.where(self.pol==0)[0]
        self.zero_pamp_index = np.where(self.pamp==0)[0]
        self.zero_spratio_index = np.where(self.spratio==0)[0]
        ...
        self.pol_array = np.delete(self.pol, self.zero_pol_index)
        self.pamp_array = np.delete(self.pamp, self.zero_pamp_index)
        self.spratio_array = np.delete(self.spratio, self.zero_spratio_index)
