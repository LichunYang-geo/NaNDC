...
from moment_tensor import mom2other, mt_to_moment, mt_to_magnitude, mt_to_gamma_delta
...
def plot_mech(savepath, filename, filetype, moment_tensor,
              pol=None, pamp=None, spratio=None, takeoff=None, azimuth=None,
              best_dc=False, additional_info=False, PTaxis=True, **kwargs):
    '''
    Creates a plot of the mechanism.
    **kwargs{color_t, color_p, edgecolor, projection, linewidth, alpha, arcces, beachball_type, view}
    '''
    ...
    beachball.plot_beachball_mpl(mt, axes1, position=(0., 0.), size=2, size_units='data', **kwargs)
    
    if pol is not None:
        if takeoff is None or azimuth is None:
            raise ImportError("Error: Missing information about azimuth and takeoff angles.")
        else:
            _add_pol(axes1, azimuth, takeoff, pol, projection=kwargs['projection'] if 'projection' in kwargs else 'lambert')
    
    if pamp is not None:
        if takeoff is None or azimuth is None:
            raise ImportError("Error: Missing information about azimuth and takeoff angles.")
        else:
            _add_pamp(axes1, azimuth, takeoff, pamp, projection=kwargs['projection'] if 'projection' in kwargs else 'lambert')
    
    if spratio is not None:
        if takeoff is None or azimuth is None:
            raise ImportError("Error: Missing information about azimuth and takeoff angles.")
        else:
            _add_spratio(axes1, azimuth, takeoff, spratio, projection=kwargs['projection'] if 'projection' in kwargs else 'lambert')
    
    # show nodal lines of best double-couple 
    if best_dc:
        _add_bestDC(axes1, moment_tensor, siz=1,
    ...
    if additional_info: _add_text(axes2, moment_tensor)

    # show P and T axis
    if PTaxis:
        _add_PTaxis(axes1, moment_tensor, 
                    kwargs['projection'] if 'projection' in kwargs else 'lambert')
        
    # save figure
    fig.savefig(os.path.join(savepath, filename + '.' + filetype), dpi=300, bbox_inches='tight', facecolor='white', transparent=False)

def plot_mech_data(savepath, filename, filetype, 
              moment_tensor, best_dc=False, PTaxis=True,
              takeoff=None, azimuth=None, pol=None, pamp=None, spratio=None, **kwargs):
    '''
    ...
    **kwargs{color_t, color_p, edgecolor, projection, linewidth, alpha, arcces, beachball_type, view}
    ...
    if pol is None:
        ax[0].text(-0.6, 0, "No P-polarity", {'size': 20})
    else:
        # pyrocke beachball fucntion
        beachball.plot_beachball_mpl(mt, ax[0], position=(0., 0.), size=2, size_units='data', **kwargs)
        ...
        if best_dc:
            _add_bestDC(ax[0], moment_tensor, 
                    ...
                    linewidth=kwargs['linewidth'] if 'linewidth' in kwargs else 2)
        # show P and T axis
        if PTaxis: _add_PTaxis(ax[0], moment_tensor, projection=kwargs['projection'] if 'projection' in kwargs else 'lambert')
    
    if pamp is None:
        ax[1].text(-0.6, 0, "No P-amplitude", {'size': 20})
    else:
        # pyrocke beachball fucntion
        beachball.plot_beachball_mpl(mt, ax[1], position=(0., 0.), size=2, size_units='data', **kwargs)
        ...
        if best_dc:
            _add_bestDC(ax[1], moment_tensor, 
                    ...
                    linewidth=kwargs['linewidth'] if 'linewidth' in kwargs else 2)
        # show P and T axis
        if PTaxis: _add_PTaxis(ax[1], moment_tensor, projection=kwargs['projection'] if 'projection' in kwargs else 'lambert')    

    if spratio is None:
        ax[2].text(-0.6, 0, "No S/P ratio", {'size': 20})
    else:
        # pyrocke beachball fucntion
        beachball.plot_beachball_mpl(mt, ax[2], position=(0., 0.), size=2, size_units='data', **kwargs)
        ...
        if best_dc:
            _add_bestDC(ax[2], moment_tensor, 
                    ...
                    linewidth=kwargs['linewidth'] if 'linewidth' in kwargs else 2)
        # show P and T axis
        if PTaxis: _add_PTaxis(ax[2], moment_tensor, projection=kwargs['projection'] if 'projection' in kwargs else 'lambert')

    # save figure
    fig.savefig(os.path.join(savepath, filename + '.' + filetype), dpi=300, bbox_inches='tight', facecolor='white', transparent=False)
    
def _add_bestDC(axes, moment_tensor, siz=1, n=181, linecolor='black', linewidth=2):
    '''
    from moment tensor (NED) to nodal lines coordinate under 'lambert projection'
    '''
    np1, np2, _, _, _ = mom2other(moment_tensor)
    s1, d1, s2, d2 = np1[0], np1[1], np2[0], np2[1]
...
def _add_text(axes, moment_tensor, fontsize=18):
    ...
    gamma, delta  = mt_to_gamma_delta(moment_tensor)     
...
def _add_PTaxis(axes, moment_tensor, projection='lambert', fontsize=22):
...
def _add_pol(axes, azimuth, takeoff, pol, projection='lambert', labelsize=100):
    ...
    for i in range(0,len(pol)):
        ...
        axes.scatter(x, y, s=labelsize, c=color, marker=marker, edgecolors='black')

def _add_pamp(axes, azimuth, takeoff, pamp, projection='lambert', labelsize=100):

    pamp = pamp/np.median(np.abs(pamp))
    for i in range(0,len(pamp)):
        rtp = np.array([[1.0 if takeoff[i] <= 90. else -1., np.deg2rad(takeoff[i]), np.deg2rad(90.-azimuth[i])]])
        # to 3D coordinates (x, y, z)
        point = beachball.numpy_rtp2xyz(rtp)
        # project to 2D with same projection as used in beachball
...
def _add_spratio(axes, azimuth, takeoff, spratio, projection='lambert', labelsize=50):
